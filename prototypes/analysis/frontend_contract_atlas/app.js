...
function syncStateFromLocation() {
  ...
  state.pageId = atlasData.shell_pages.some((page) => page.page_id === locationState.pageId)
    ...
    : DEFAULT_PAGE;
  state.scenarioId = atlasData.continuity_scenarios.some(
    (item) => item.scenario_id === locationState.scenarioId,
  )
...
function makeMetricCard(label, value) {
  ...
  row.append(
    ...
    createElement("span", "metric-value", value),
  );
  return row;
}
...
function renderCalmStage() {
  ...
  card.append(
    ...
    createElement(
      "p",
      "card-copy",
      "Quiet, summary-first, and support-budgeted. External handoffs and detail work stay parent-bound.",
    ),
  );
  ...
  const blocks = [
    [
      "context-bar",
      "CONTEXT_BAR",
      "Current object, settlement posture, typed notices, and return path.",
    ],
    [
      "decision-summary",
      "DECISION_SUMMARY",
      "Dominant question, lawful state, and highest-signal decision context.",
    ],
    [
      "action-strip",
      "ACTION_STRIP",
      "Exactly one dominant action plus fail-closed no-safe-action posture.",
    ],
    [
      "detail-drawer",
      "DETAIL_DRAWER",
      "Evidence, packet, authority, drift, focus, and twin-lens support modules.",
    ],
  ];
...
function renderPortalStage() {
  ...
  card.append(
    ...
    createElement(
      "p",
      "card-copy",
      "One primary task column, plain-language status, and contextual support that never leaks internal workflow state.",
    ),
  );
  ...
  [
    ...
    [
      "portal-status-hero",
      "STATUS_HERO",
      "Next required client action with typed status language.",
    ],
    [
      "portal-primary-action",
      "PRIMARY_ACTION",
      "Single lawful CTA for documents, approvals, onboarding, or help.",
    ],
    [
      "portal-support-panel",
      "PROMOTED_SUPPORT_REGION",
      "Stacked support content beneath the primary task.",
    ],
    [
      "portal-current-artifact",
      "CURRENT_ARTIFACT",
      "Current client-visible artifact or handoff target.",
    ],
    [
      "portal-history-list",
      "HISTORY",
      "Historical items remain distinct from the current artifact.",
    ],
...
function renderGovernanceStage() {
  ...
  card.append(
    ...
    createElement(
      "p",
      "card-copy",
      "Dense admin workspaces still respect one dominant question, one promoted auxiliary surface, and inline typed recovery.",
    ),
  );
  ...
  left.append(
    createElement("strong", "", "INVENTORY_RAIL"),
    createElement("span", "", "Filter slice, object selection, and route-stable worklist."),
  );
  stage.append(left);
  ...
  middle.append(
    createElement("strong", "", "WORKSPACE_CANVAS"),
    createElement(
      "span",
      "",
      "Primary diff, mutation, simulation, or audit canvas driven by the selected governance object.",
    ),
  );
  stage.append(middle);
  ...
  right.append(
    createElement("strong", "", "AUDIT_SIDECAR"),
    createElement(
      "span",
      "",
      "One promoted auxiliary surface for diff, audit, blast radius, or chain detail.",
    ),
  );
  stage.append(right);
...
function renderShellPage(pageId) {
  ...
  lawCard.append(
    ...
    createElement(
      "p",
      "card-copy",
      `${foundation.interaction_layer_contract} with ${foundation.behavior_contract.selector_profile}.`,
    ),
  );
  const chips = createElement("div", "chip-row");
  chips.append(
  ...
  lawCard.append(
    makeMetricCard(
      ...
      foundation.behavior_contract.history_presentation_policy,
    ),
    makeMetricCard("Preview surface", foundation.behavior_contract.preview_surface_policy),
    makeMetricCard(
      "Notification surface",
      foundation.behavior_contract.notification_surface_policy,
    ),
  ...
  selectorsCard.append(
    ...
    createElement(
      "p",
      "card-copy",
      `Profile ${profile.profile_id} contributes ${profile.selector_entries.length} semantic anchors.`,
    ),
  );
  const selectorChips = createElement("div", "chip-row");
  profile.selector_entries
    .slice(0, 16)
    .forEach((entry) => selectorChips.append(createChip(entry.selector)));
  ...
  routes.forEach((route) => {
    ...
    [
      route.dominant_action_policy,
      route.promoted_support_region_policy,
      `Focus order: ${route.focus_order.join(" -> ")}`,
    ].forEach((item) => {
      const li = document.createElement("li");
      li.textContent = item;
      list.append(li);
...
function renderContinuityPage() {
  ...
  const scenario = atlasData.continuity_scenarios.find(
    (item) => item.scenario_id === state.scenarioId,
  );
  const fragment = document.createDocumentFragment();
  fragment.append(makeHeader(page));
  ...
  chipRow.append(
    ...
    createChip(
      scenario.announcement_posture === "assertive"
        ? "assertive live region"
        : "polite live region",
      scenario.announcement_posture === "assertive" ? "chip-danger" : "chip-success",
    ),
  );
  ...
  invariants.append(
    createElement("p", "kicker", "Preserved invariants"),
    createElement("h3", "section-title", "What must survive"),
  );
  ...
  focus.append(
    createElement("p", "kicker", "Focus restoration"),
    createElement("h3", "section-title", "Fallback order"),
  );
  ...
  supportDemo.append(
    ...
    createElement(
      "p",
      "card-copy",
      "Open the support region, then close it. Focus returns to the invoker to model parent-bound support law.",
    ),
  );
  ...
  panel.append(
    ...
    createElement(
      "p",
      "card-copy",
      "This panel stands in for compare, audit, packet review, or authority support windows.",
    ),
  );
...
async function bootstrap() {
  ...
  updateLocation(
    {
      pageId: locationState.pageId || DEFAULT_PAGE,
      scenarioId: locationState.scenarioId || "refresh_preserves_same_object",
    },
    true,
  );
