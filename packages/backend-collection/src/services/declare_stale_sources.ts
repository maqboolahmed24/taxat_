...
export function declareStaleSources(input: {
  collection_boundary: CollectionBoundaryRecord;
  produced_at: string;
  reason_code?: "FRESHNESS_POLICY_VIOLATION" | "REVISION_DRIFT" | "SCHEMA_DRIFT" | "STALE_AT_CUTOFF";
  ...
  return boundary.source_boundaries
    .filter((sourceBoundary) => sourceBoundary.boundary_disposition === "STALE_AT_CUTOFF")
    .map((sourceBoundary) =>
      buildSourceDomainDeclarationRecord({
        ...
        reason_code: input.reason_code ?? "STALE_AT_CUTOFF",
