...
test("checked-in secret alias, hierarchy, rotation, inventory, and viewer artifacts stay aligned with the builder", async () => {
  const persistedAliasCatalog = await readJson(["config", "secrets", "secret_alias_catalog.json"]);
  ...
  const sampleRun = await readJson<{
    secretRootTopologyLedger: ReturnType<typeof createSecretRootTopologyLedgerViewModel>;
  ...
  expect(sampleRun.secretRootTopologyLedger).toEqual(createSecretRootTopologyLedgerViewModel());
});
...
test("alias families remain unique and namespace-partitioned", () => {
  ...
  expect(
    catalog.aliases.every((alias) =>
      alias.namespace_refs.length <= 1 ? true : alias.store_ref_template.includes("{namespace}"),
    ),
  ).toBe(true);

  const authorityAliases = catalog.aliases.filter(
    (alias) => alias.secret_class === "AUTHORITY_CLIENT_SECRET",
  );
  expect(
    authorityAliases.every((alias) =>
      alias.namespace_refs.every((namespaceRef) => namespaceRef.includes("authority")),
    ),
  ).toBe(true);
});
