...
test("checked-in access policy matrix stays aligned with the builder", async () => {
  const persistedMatrix = await readJson(["config", "secrets", "access_policy_matrix.json"]);
...
test("least-privilege separation remains explicit across read, decrypt, rotate, and policy capabilities", () => {
  ...
  const projectionGrant = matrix.grants.find((row) => row.role_ref === "role.projection_service");
  expect(projectionGrant?.capabilities.read_secret).toBe("DENY");
  expect(projectionGrant?.capabilities.decrypt_unwrap).toBe("DENY");

  const supportGrant = matrix.grants.find((row) => row.role_ref === "role.support_adapter");
  expect(supportGrant?.alias_refs.some((aliasRef) => aliasRef.includes("authority.hmrc"))).toBe(
    false,
  );

  const breakGlassGrant = matrix.grants.find((row) => row.role_ref === "role.break_glass_operator");
