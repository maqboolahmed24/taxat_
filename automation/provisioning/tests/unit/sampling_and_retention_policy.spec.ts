...
test("every signal family stays bound to a backend, retention class, and scrub posture while forbidden fields remain structurally blocked", () => {
  ...
  const samplingRefs = new Set(samplingPolicy.policy_rows.map((row) => row.sampling_policy_ref));
  ...
  expect(
    samplingPolicy.policy_rows.every((row) =>
      ["default", "elevated_debug", "incident", "privacy_constrained"].every((posture) =>
  ...
  expect(
    logPolicy.family_rows.every((row) =>
      logPolicy.global_hash_only_fields.every((field) => row.hash_only_fields.includes(field)),
    ),
  ).toBe(true);
});
