...
test("channel matrix keeps durable outboxes, authenticated ingress, and rebuildable broker law explicit", () => {
  ...
  expect(callbackIngress?.inbox_mode).toBe("AUTHENTICATED_DEDUPE_INBOX_REQUIRED");
  expect(callbackIngress?.truth_boundary_statement).toContain("before legal-state mutation");
  expect(callbackIngress?.truth_boundary_statement).toContain("authenticated dedupe inbox");
  ...
  expect(stageDispatch?.outbox_identity_fields).toEqual(
    expect.arrayContaining(["tenant_id", "manifest_ref", "stage_code", "command_receipt_ref"]),
  );
