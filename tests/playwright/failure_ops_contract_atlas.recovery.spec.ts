...
test("failure lifecycle state transitions keep closure posture explicit", async ({ page }) => {
  ...
  await expect(page.getByTestId("failure-dashboard-projection")).toContainText(
    "bounded exception path",
  );
  await expect(page.getByTestId("failure-evidence-inspector")).toContainText(
    "Accepted risk is explicit, bounded, and reviewable",
  );
  ...
  await expect(page.getByTestId("failure-evidence-inspector")).toContainText(
    "Resolution is terminal only because typed artifacts say it is",
  );
});
...
test("responsive inspector collapse preserves anchors", async ({ page }) => {
  ...
  await expect
    .poll(async () => page.evaluate(() => document.documentElement.dataset.inspectorLayout))
    .toBe("stacked");
  ...
  await expect
    .poll(async () => page.evaluate(() => document.documentElement.dataset.stackLayout))
    .toBe("stacked");
  await expect(page.getByTestId("failure-evidence-inspector")).toBeVisible();
});
...
test("failure lifecycle screenshot baseline", async ({ page }) => {
  ...
  await expect(page.getByTestId("failure-ops-contract-atlas")).toHaveScreenshot(
    "failure-ops-contract-atlas-failure-lifecycle.png",
    {
      animations: "disabled",
      fullPage: true,
    },
