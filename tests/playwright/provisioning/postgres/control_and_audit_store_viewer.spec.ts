...
test("supports keyboard selection across stores, roles, and restore policy rows", async ({
  ...
  const roleButton = page.locator(".postgres-role-row").filter({ hasText: "Audit append writer" });
  await roleButton.focus();
  await page.keyboard.press("Enter");

  await expect(page.locator("#drawer-body")).toContainText("role.pg.audit.append_writer");
  ...
  const policyButton = page.locator(".postgres-policy-row").filter({ hasText: "PITR and WAL" });
  ...
  await expect(page.locator("#drawer-body")).toContainText("RESTORE_EVIDENCE_BOUND");
});
