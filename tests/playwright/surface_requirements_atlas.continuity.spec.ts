...
async function gotoAtlas(
  page,
  hash = "#page=continuity&scenario=inline_rebase&record=collaboration_staff_workspace",
) {
  await page.goto(`${atlasPath}${hash}`);
  await expect(page.getByTestId("surface-requirements-atlas")).toBeVisible();
}

test("continuity scenarios switch and support focus returns to the parent trigger", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("evidence-inspector")).toContainText(
    "Native scene restore under legality checks",
  );
});
...
test("continuity screenshot baseline", async ({ page }) => {
  ...
  await expect(page.getByTestId("surface-requirements-atlas")).toHaveScreenshot(
    "surface-requirements-atlas-continuity.png",
    {
      animations: "disabled",
      fullPage: true,
    },
